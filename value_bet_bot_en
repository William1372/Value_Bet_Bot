...
SPORT = "soccer_epl"  # e.g. "soccer_denmark_superliga"
REGION = "eu"
MARKETS = "h2h"
...
ODDS_JUMP_THRESHOLD = 0.20  # how much odds must change before sending jump alert

CHECK_INTERVAL = 900        # every 15 minutes
VALUE_SEND_EVERY = 3        # send value bets every 3rd iteration

iteration_counter = 0
previous_odds = {}          # stores previous odds for jump detection
...
# ------------------------------------------------------------
# FAVORITE + BEST PRICE ON FAVORITE
# ------------------------------------------------------------
def get_favorite_and_best_price(bookmakers, home, away):
    ...
    favorite = min(outcome_avgs, key=lambda k: outcome_avgs[k])  # lowest odds = favorite
...
"""
    ...
    if v_home and v_home >= VALUE_THRESHOLD:
        ft = ""
        if favorite == "home":
            ft = f"Favorite: {favorite.upper()} – best price {fav_best_price} at {fav_best_book}"
